...
class Hotel extends Bookable
{
    ...
    {
        return $this->hasOne("Modules\Hotel\Models\HotelCategory", "id", 'category_id')->with(['translations']);
    }
    /**
     * Get Institutional
     *
     * @return \Illuminate\Database\Eloquent\Relations\HasOne
     */
    public function institutional_hotel()
    {
        return $this->hasOne("Modules\Hotel\Models\HotelInstitutional", "id", 'institutional_id')->with(['translations']);
    }
    ...
    {
        return $this->belongsToMany(HotelCategory::class, 'bravo_hotel_category_relationships', 'hotel_id', 'category_id');
    }
    public function institutions()
    {
        return $this->belongsToMany(HotelInstitutional::class, 'bravo_hotel_institutional_relationships', 'hotel_id', 'institutional_id');
    }
    ...
    public function getListHotels($category_id = false,$institutional_id = false, $location = false)
    {
        $model_hotel = parent::query()->select("bravo_hotels.*");
        $model_hotel->where("bravo_hotels.status", "publish");

        if (!empty($category_id)) {
            $model_hotel->join('bravo_hotel_category_relationships as pcs', 'pcs.hotel_id', "bravo_hotels.id")->where('pcs.category_id', $category_id);
        }
        if (!empty($institutional_id)) {
            $model_hotel->join('bravo_hotel_institutional_relationships as pcs', 'pcs.hotel_id', "bravo_hotels.id")->where('pcs.institutional_id', $institutional_id);
        }
        if (!empty($location)) {
        ...
        if (!empty($orderby = request()->query("orderby"))) {
            switch ($orderby) {
                case "new":
                    ...
                    break;
                case "old":
                    $model_hotel->orderBy("id", "asc");
                    break;
                case "price_high":
                    $model_hotel->orderBy("price", "desc");
                    break;
                case "price_low":
                    $model_hotel->orderBy("price", "asc");
                    break;
                case "name_high":
                    $model_hotel->orderBy("title", "asc");
                    break;
                case "name_low":
                    $model_hotel->orderBy("title", "desc");
                    break;
                default:
        ...
        return $model_hotel->with(['location', 'hasWishList', 'translations', 'categories'])->paginate($limit);
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $discount_by_night = max(1,ceil(($end_date->getTimestamp() - $start_date->getTimestamp()) / DAY_IN_SECONDS ) + 1 );
        ...
        if ($this->discount_by_night and !empty($this->discount_by_night)) {
            foreach ($this->discount_by_night as $type) {
                if ($type['from'] <= $discount_by_night and (!$type['to'] or $type['to'] >= $discount_by_night)) {
                    ...
                    $discount += $type_total;
                    $type['total'] = $type_total;
                    $discount_by_night[] = $type;
                }
            }
        }
        // children_age
        $children_age = [];
        $children_age_input = $request->input('children_age');
            foreach ($children_age_input as $k =>  $type) {
                    $children_age[] = $type;
            }
        $children_age= json_encode($children_age);
        ...
        $booking->children_age= $children_age;
        ...
        if ($check) {
            ...
            $booking->addMeta('discount_by_night', $discount_by_night);
            $booking->addMeta('children_age', $children_age);
    ...
    public function getBookingData()
    {
        ...
        if (!empty($this->discount_by_night)) {
            $booking_data['discount_by_night'] = $this->discount_by_night;
            if (!empty($booking_data['discount_by_night'])) {
                foreach ($booking_data['discount_by_night'] as $k => &$type) {
                    $type['from']=$type['from'];
                    $type['to']=$type['to'];
                    $type['amount']=$type['amount'];
                    $type['type']=($type['type'] == "fixed") ? "" : "%"; ;
                }
            }

            $booking_data['discount_by_night'] = array_values((array)$booking_data['discount_by_night']);
        }
    ...
    public function getReviewDataAttribute()
    {
        ...
        $list_data_rate = $this->reviewClass::selectRaw('COUNT( CASE WHEN rate_number = 10 THEN rate_number ELSE NULL END ) AS rate_10,
                                                            COUNT( CASE WHEN rate_number = 9 THEN rate_number ELSE NULL END ) AS rate_9,
                                                            COUNT( CASE WHEN rate_number = 8 THEN rate_number ELSE NULL END ) AS rate_8,
                                                            COUNT( CASE WHEN rate_number = 7 THEN rate_number ELSE NULL END ) AS rate_7,
                                                            COUNT( CASE WHEN rate_number = 6 THEN rate_number ELSE NULL END ) AS rate_6,
                                                            COUNT( CASE WHEN rate_number = 5 THEN rate_number ELSE NULL END ) AS rate_5,
    ...
    public function getRoomsAvailability($filters = [])
    {
        ...
        foreach ($rooms as $room) {
            if ($room->isAvailableAt($filters)) {
                ...
                if ($room->discount_by_night and !empty($room->discount_by_night)) {
                    foreach ($room->discount_by_night as $type) {
                        if ($type['from'] <= $discount_by_night and (!$type['to'] or $type['to'] >= $discount_by_night)) {
                            ...
                            switch ($type['type']) {
                                case "fixed":
                                    ...
                                    break;
                                case "percent":
                                    $type_total = $room->tmp_price / 100 * $type['amount'];
                                    break;
                            }
                            dd($type_total);
                            $room->tmp_price -= $type_total;
                            $discount += $type_total;
                            $type['total'] = $type_total;
                            $discount_by_night[] = $type;
                        }
                ...
                $res[] = [
                    ...
                    'discount_html'    => format_money($discount) ,
    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($institutional_id = $request->query('institutional_id'))) {
            $model_hotel->join('bravo_hotel_institutional_relationships as pcs', 'pcs.hotel_id', "bravo_hotels.id")->where('pcs.institutional_id', $institutional_id);
        }
        ...
        $list = $model_hotel->with(['location', 'hasWishList', 'translations', 'termsByAttributeInListingPage', 'categories'])->paginate($limit);
        return $list;
    }
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        if ($forSingle) {
            ...
            $data['category'] = HotelCategory::selectRaw("id,name")->find($this->category_id) ?? null;
            $data['institutional'] = HotelInstitutional::selectRaw("id,name")->find($this->institutional_id) ?? null;
            ...
            $data["discount_by_night"] = null;
            if (!empty($this->discount_by_night)) {
                $data["person_types"] = $this->discount_by_night;
            }
    ...
    static public function getFiltersSearch()
    {
        ...
        $category = HotelCategory::selectRaw("id,name,slug")->where('status', 'publish')->with(['translations'])->get()->toTree();
        $institutional = HotelInstitutional::selectRaw("id,name,slug")->where('status', 'publish')->with(['translations'])->get()->toTree();
        return [
            [
            ...
            [
                "title"    => __("Institutional"),
                "field"    => "cat_id",
                "position" => "3",
                "data" => $institutional->map(function ($institutional) {
                    return $institutional->dataForApi();
                })
            ],
            [
                "title"    => __("Attributes"),
    ...
    {
        return $this->belongsTo('Modules\Hotel\Models\HotelCategory', 'category_id', 'id');
    }
    public function Institutional()
    {
        return $this->belongsTo('Modules\Hotel\Models\HotelInstitutional', 'institutional_id', 'id');
    }
}
