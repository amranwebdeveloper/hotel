    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                @if ($ready_for_update)
                    <div class="panel">
                        <div class="panel-title"><strong>{{ __('Update Tatil Benim') }}</strong></div>
                        <div class="panel-body">

                            @if (($updater_latest_version = setting_item('updater_latest_version')) and
                                version_compare(config('app.version'), $updater_latest_version, '='))
                                <p class="alert-success alert">
                                    <strong>{{ __('You are using newest version of Tatil Benim: :version', ['version' => $updater_latest_version]) }}</strong>
                                </p>
                            @endif

                            <p><strong>{{ __('Your license key: :key', ['key' => setting_item('envato_license_key')]) }}</strong>
                            </p>
                            @if ($last_check_update = setting_item('last_check_update'))
                                <p>{{ __('Last check for update: :date', ['date' => display_datetime((int) $last_check_update)]) }}
                                </p>
                            @endif

                            @if ($updater_last_success = setting_item('updater_last_success'))
                                <p>{{ __('Last update success: :date', ['date' => display_datetime((int) $updater_last_success)]) }}
                                </p>
                            @endif
                            ...
                            @if (($updater_latest_version = setting_item('updater_latest_version')) and
                                ...
                                <p><label><input type="checkbox" class="check_installation_term">
                                        {{ __('I already back up all files and database') }}</label></p>
                                <button type="submit"
